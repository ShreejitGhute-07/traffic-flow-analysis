...
#     if key == 13:  # Enter
#         lanes.sort(key=lambda l: l[1])  # Sort top-to-bottom
...
# counted_vehicles = set()  # Store IDs already counted
# results_data = []
# frame_count = 0
...
#                 continue  # Skip if already counted in a lane
...
#                     results_data.append([int(vid), lane_id, frame_count, int(time.time())])
#                     break  # Stop checking other lanes
...
#     df.to_csv("output/vehicle_counts.csv", index=False, mode='w')
#     print(f"✅ CSV saved with {len(results_data)} rows")
# else:
